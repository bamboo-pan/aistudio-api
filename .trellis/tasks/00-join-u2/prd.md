...
`u2` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/u2/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/u2/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee u2` to
...
python ./.trellis/scripts/task.py archive 00-join-u2
```
