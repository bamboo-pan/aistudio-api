...
async def request_json(http, method, path, payload):
    response = await http.request(method, path, json=payload, timeout=360)
    try:
        body = response.json()
    except Exception:
...
async def main():
    ...
    print("live_timeout_replay_default", settings.timeout_replay)
    if settings.timeout_replay != 137:
        return 3
    if auth_path is None or not auth_path.exists():
        return 2
    ...
    client = AIStudioClient(port=int(os.environ.get("AISTUDIO_CAMOUFOX_PORT", "19457")))
    ...
    forwarded_timeouts = []
    original_generate_image = client.generate_image

    async def generate_image_with_probe(**kwargs):
        forwarded_timeouts.append(kwargs.get("timeout", "missing"))
        return await original_generate_image(**kwargs)

    client.generate_image = generate_image_with_probe
    image_ok = False
    try:
        try:
            await client.warmup()
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as http:
            image_status, image_body, image_text = await request_json(
                ...
                "/v1/images/generations",
                {
                    ...
                    "prompt": "A simple blue square centered on a plain white background. No text.",
                    "size": "512x512",
                    "n": 1,
            ...
            image_ok = image_status == 200 and image_b64_len > 0 and forwarded_timeouts == [242]
            print("live_image_status", image_status)
            print("live_image_returned", image_b64_len > 0)
            print("live_image_b64_len", image_b64_len)
            print("live_forwarded_timeouts", forwarded_timeouts)
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="aistudio-api-u3-live-timeout-", dir="/home/bamboo"))
    ...
    env["AISTUDIO_CAMOUFOX_PORT"] = "19457"
    env["AISTUDIO_LOGIN_CAMOUFOX_PORT"] = "19458"
    env["AISTUDIO_TIMEOUT_CAPTURE"] = "120"
    env["AISTUDIO_TIMEOUT_REPLAY"] = "137"
