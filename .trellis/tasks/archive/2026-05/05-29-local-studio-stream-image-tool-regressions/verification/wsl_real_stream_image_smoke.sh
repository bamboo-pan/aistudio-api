...
RUN_ROOT="${AISTUDIO_SMOKE_RUN_ROOT:-/home/bamboo/aistudio-api-stream-image-$(date +%Y%m%d-%H%M%S)}"
...
cat >"$RUN_ROOT/real_stream_image_smoke.py" <<'PY'
import asyncio
import json
import os
import re
...
async def load_models(client: httpx.AsyncClient) -> tuple[str, str, str]:
    payload = {"provider_type": "google-ai-studio", "interface_mode": "responses", "timeout": 600}
    last_error = ""
    deadline = time.monotonic() + 420
    while time.monotonic() < deadline:
        try:
            response = await client.post(f"{BASE_URL}/api/local-studio/models", json=payload, timeout=90)
            if response.status_code == 200:
                data = response.json()
                ...
                default_image_model = choose_model(image_models, ["gemini-3-pro-image-preview", "gemini-3.1-flash-image-preview"])
                screenshot_image_model = "gemini-3.1-flash-image-preview" if "gemini-3.1-flash-image-preview" in model_ids(image_models) else default_image_model
                note(f"selected chat_model={chat_model} image_model={screenshot_image_model}")
                return chat_model, default_image_model, screenshot_image_model
...
async def stream_local_studio_chat(client: httpx.AsyncClient, payload: dict) -> tuple[list[tuple[int, dict]], str]:
    ...
    buffer = ""
    chunk_index = 0
    async with client.stream("POST", f"{BASE_URL}/api/local-studio/chat", json=payload, timeout=600) as response:
        body_prefix = ""
        if response.status_code >= 400:
            body_prefix = await response.aread()
            raise AssertionError(f"chat stream failed HTTP {response.status_code}: {body_prefix[:1200]!r}")
...
def local_payload(chat_model: str, image_model: str, message: str) -> dict:
    return {
...
async def verify_api_stream_text(client: httpx.AsyncClient, chat_model: str, image_model: str) -> None:
    note("API stream text check")
    payload = local_payload(
        chat_model,
        image_model,
        "Answer normally without making an image. Write three short numbered sentences about stream verification.",
    )
    events, raw_text = await stream_local_studio_chat(client, payload)
    require("Requested entity was not found" not in raw_text, "unexpected image-model not-found text in text stream")
    deltas = [(index, event) for index, event in events if event.get("type") == "local_studio.delta" and str(event.get("content") or "").strip()]
    ...
    require(not errors, f"stream emitted errors: {errors[:2]}")
    require(deltas, "text stream did not emit local_studio.delta content")
    require(completions, "text stream did not emit local_studio.completed")
    require(deltas[0][0] < completions[-1][0], "first text delta did not arrive before completion chunk")
    note(f"API stream text ok: first_delta_chunk={deltas[0][0]} completed_chunk={completions[-1][0]}")


async def verify_api_image_tool(client: httpx.AsyncClient, chat_model: str, image_model: str) -> None:
    note("API image-tool check")
    payload = local_payload(
        chat_model,
        image_model,
        "Use the image_generation tool to create a simple flat test image: a blue square centered on a white background.",
    )
    events, raw_text = await stream_local_studio_chat(client, payload)
    require("Requested entity was not found" not in raw_text, "image generation still returned requested-entity-not-found")
    errors = [event for _, event in events if event.get("type") == "error" or event.get("error")]
    require(not errors, f"image stream emitted errors: {errors[:2]}")
    completions = [event for _, event in events if event.get("type") == "local_studio.completed"]
    require(completions, "image stream did not complete")
    conversation = completions[-1].get("conversation") or {}
    ...
    note(f"API image-tool ok: requested_image_model={image_model} asset_bytes={len(asset.content)}")
...
async def verify_ui(chat_model: str, image_model: str) -> None:
    ...
    async with async_playwright() as playwright:
        ...
        await textarea.fill("Answer normally without making an image. Write three short stream-verification words separated by commas.")
        await page.locator("#studio-page button.send").click()
        await page.wait_for_function(
            """
            () => {
              const pending = Array.from(document.querySelectorAll('.local-studio-message.is-pending')).some(el => getComputedStyle(el).display !== 'none');
              const bodies = Array.from(document.querySelectorAll('.local-studio-message.is-assistant:not(.is-pending) .msg-body.markdown-body'));
              const visibleText = bodies.map(el => getComputedStyle(el).display !== 'none' ? el.textContent.trim() : '').filter(Boolean).join(' ');
              return pending && visibleText.length > 0;
            }
            """,
            timeout=180000,
        )
        ...
        ui_text = await page.locator(".local-studio-message.is-assistant:not(.is-pending) .msg-body.markdown-body").last.text_content()
        require(ui_text and ui_text.strip(), "UI text stream completed without visible assistant text")
        await textarea.fill("Use the image_generation tool to create a simple flat test image: a green circle on white background.")
        await page.locator("#studio-page button.send").click()
        await page.wait_for_function(
            """
        ...
        await page.screenshot(path=str(ARTIFACT_ROOT / "local-studio-smoke.png"), full_page=True)
        await browser.close()
        note("browser UI ok")
...
async def main() -> None:
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        chat_model, ui_image_model, screenshot_image_model = await load_models(client)
        await verify_api_stream_text(client, chat_model, ui_image_model)
        await verify_api_image_tool(client, chat_model, screenshot_image_model)
        await verify_request_logs(client)
    await verify_ui(chat_model, ui_image_model)
...
if ! python "$RUN_ROOT/real_stream_image_smoke.py"; then
