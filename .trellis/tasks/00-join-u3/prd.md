...
`u3` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/u3/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/u3/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee u3` to
...
python ./.trellis/scripts/task.py archive 00-join-u3
```
