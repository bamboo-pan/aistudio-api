...
- Local Studio has two provider kinds: built-in `google-ai-studio` and custom `openai`. The built-in Google provider is the default, derives an internal `/v1` or `/v1beta` base URL from the current request, and must not require or forward a user-entered token. Custom OpenAI providers require a valid base URL plus single-line token before model loading or chat.
- `base_url` must point at the selected provider root when the provider kind is `openai`: `/v1` for OpenAI Chat, Responses, and Claude-compatible routes; `/v1beta` or an equivalent Gemini root for Gemini routes. Route helpers append `/models`, `/chat/completions`, `/responses`, `/models/{model}:generateContent`, `/models/{model}:streamGenerateContent`, or `/messages` according to `interface_mode`.
...
- Responses image-tool streams may expose the model's selection as `response.output_item.done` with a `function_call` item before image generation completes. Local Studio must translate that event into a visible `local_studio.delta.thinking` progress update before `local_studio.completed`; otherwise the browser UI appears to return the image result all at once even though the upstream stream already announced tool selection.
...
- Good: Responses image tool emits a `function_call` output item for `image_generation` before the generated image is ready; Local Studio emits a `local_studio.delta` with tool progress while the UI is still pending, then persists both the tool progress and final image trace in the completed assistant thinking.
...
- Real stream image-tool path: WSL API smoke must verify a Google Local Studio Responses image-tool stream emits tool-progress `local_studio.delta.thinking` before `local_studio.completed`, and browser UI smoke must observe the same assistant thinking update while the pending indicator is still visible.
