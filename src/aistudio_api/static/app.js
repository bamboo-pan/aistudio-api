...
function app(){return{
  ...
  go(v,opts={}){this.view=v;this.sidebarOpen=false;if(opts.syncHash!==false&&window.location.hash!==`#${v}`)window.location.hash=v;if(v==='dashboard')this.loadStats();if(v==='accounts'){this.loadAccounts();this.loadRotation()}if(v==='images')this.ensureImageDefaults()},
  ...
  clearImageEditSession(){this.imageReferences=[];this.imageBaseImage=null;this.imageConversation=[];this.imageReferenceError='';this.showToast('已开启新会话')},
