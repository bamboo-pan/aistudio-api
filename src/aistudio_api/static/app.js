...
function app(){return{
  ...
  imageModel:'',imagePrompt:'',imageSize:'1024x1024',imageCount:1,imageBusy:false,imageError:'',imageResults:[],imageHistory:[],imageHistorySelection:{},imageHistoryDeleting:false,imageHistoryError:'',imageLastRequest:null,
  toast:{show:false,msg:'',t:null},

  init(){this.loadImageHistory();this.loadModels();this.loadStats();this.loadAccounts();this.loadRotation();document.addEventListener('click',()=>this.closeSelect());document.addEventListener('keydown',(event)=>this.handleSelectKeydown(event))},
  ...
  async generateImage(){const prompt=this.imagePrompt.trim();if(!prompt||this.imageBusy)return;this.ensureImageDefaults();if(!this.imageModel){this.imageError='没有可用的图像模型';return}const count=this.normalizeImageCount();this.imageBusy=true;this.imageError='';this.imageHistoryError='';this.imageResults=[];const body={model:this.imageModel,prompt,size:this.imageSize,n:count,response_format:this.imageResponseFormat};this.imageLastRequest={...body};try{const data=await this.fetchJson('/v1/images/generations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});this.imageResults=(data.data||[]).map((item,index)=>({...item,url:this.imageUrl(item),prompt,model:this.imageModel,size:this.imageSize,index:index+1,created:data.created||Date.now()/1000}));const historyItems=this.imageResults.map((item,index)=>this.lightweightImageItem(item,index)).filter(Boolean);if(this.imageResults.length){this.imageHistory=[...historyItems,...this.imageHistory].slice(0,24);this.pruneImageSelection();this.saveImageHistory()}else{this.imageError='没有返回图片'}this.showToast(`生成完成：${this.imageResults.length} 张`)}catch(error){this.imageError=this.errMsg(error,'生成失败');this.showToast('生成失败：'+this.imageError)}finally{this.imageBusy=false}},
